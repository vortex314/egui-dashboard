...
pub fn payload_encode_cbor<X>( v: X) -> Vec<u8>
// where X: Encode<()>,
{
...
pub fn payload_decode_cbor<'a,T>(v: &'a Vec<u8>) -> Result<T, decode::Error>
where T : Decode<'a,()>
{
...
pub fn payload_display_cbor(v: &Vec<u8>) -> String {
...
pub fn payload_as_f64_cbor (payload: &Vec<u8>) -> Result<f64, decode::Error> {
...
pub fn payload_as_bool_cbor (payload: &Vec<u8>) -> Result<bool, decode::Error> {
    let mut decoder = Decoder::new(payload);
    let v =  decoder.tokens().collect::<Result<Vec<Token>, _>>()?;
    if v.len() != 1 {
